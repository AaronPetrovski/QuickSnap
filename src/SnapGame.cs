...
namespace CardGames
{
    public class SnapGame
    {
		...
		{ //Fetch the next batch of UI interaction SwinGame.ProcessEvents(); 
			if (SwinGame.KeyTyped(KeyCode.vk_SPACE)) { myGame.FlipNextCard(); }

			if (myGame.IsStarted)
			{
				if (SwinGame.KeyTyped(KeyCode.vk_LSHIFT) && SwinGame.KeyTyped(KeyCode.vk_RSHIFT))
				{
					//TODO: add sound effects } else if (SwinGame.KeyTyped (KeyCode.vk_LSHIFT)) { myGame.PlayerHit (0); } 
				}
